...
const ReviewList = () => {
  const { reviews, loading, error, updateStatus, isUpdating, page, setPage, totalItems, hasNextPage, hasPrevPage } = useReviews();
  ...
  return (
    <div className="p-2 md:p-4 mx-auto w-full max-w-[1400px]">
      ...
      <div className="space-y-4 mb-12">
        {reviews.length > 0 ? (
          reviews.map((review) => (
            <div key={review.id} className="border p-5 md:p-6 rounded-xl md:rounded-xl bg-white shadow-sm border-gray-100  hover:shadow-md transition-all">
              ...
              <div className="flex justify-between items-center gap-4">
                ...
                <div className="flex flex-col gap-1 min-w-0">
                  ...
                  <div className="flex gap-0.5">
                    {[...Array(5)].map((_, i) => (
                      <RiStarFill key={i} size={14} className={i < (review?.rating || 0) ? "text-primary" : "text-slate-100"} />
                    ))}
                  </div>
                </div>
          ...
          <div className="text-center py-20 bg-slate-50 rounded-[2rem] border-2 border-dashed border-slate-100">
             <p className="font-black uppercase text-[10px] tracking-[0.2em] text-gray-500">No feedback available yet.</p>
          </div>
        )}
      </div>
        ...
        <div className="fixed inset-0 z-[1000] flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm">
          <div className="bg-white w-full max-w-[400px] rounded-[2rem] p-8 shadow-2xl flex flex-col items-center text-center animate-in fade-in zoom-in duration-200">
            ...
            <p className="text-slate-500 text-sm font-medium mb-8 leading-relaxed">
              ...
              {pendingAction?.currentStatus ? ' It will be hidden from the website.' : ' It will be visible to everyone.'}
            </p>
            <div className="flex gap-3 w-full">
              <button 
                onClick={() => setShowModal(false)} 
                className="flex-1 py-4 text-[10px] font-black uppercase tracking-widest text-gray-500 bg-slate-50 rounded-2xl hover:bg-slate-100 transition-colors"
              >
                Cancel
              </button>
              <button 
                onClick={handleStatusUpdate} 
                className={`flex-1 py-4 text-[10px] font-black uppercase tracking-widest text-white rounded-2xl shadow-lg transition-all active:scale-95 ${
                  pendingAction?.currentStatus ? 'bg-rose-500 shadow-rose-200' : 'bg-green-500 shadow-green-200'
                }`}
              >
                Confirm
              </button>
      ...
      <div className="flex flex-col items-center justify-center gap-5 py-8 border-t border-slate-50">
        <div className="flex items-center gap-1.5 md:gap-3">
          <button 
            ...
            disabled={page === 1} 
            className="p-2 md:p-3 rounded-xl md:rounded-2xl border border-slate-200 disabled:opacity-30 bg-white hover:border-slate-900 transition-all cursor-pointer shadow-sm"
          >
          ...
          <div className="min-w-[110px] md:min-w-[140px] flex items-center justify-center bg-slate-900 px-4 py-3 md:py-3.5 rounded-xl md:rounded-2xl shadow-xl">
              <span className="text-[9px] md:text-[10px] font-black uppercase tracking-[0.1em] md:tracking-[0.2em] text-white whitespace-nowrap">
