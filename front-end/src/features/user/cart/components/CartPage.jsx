...
import { MapPin, CreditCard, ShieldCheck, Truck, Clock, ArrowRight, Minus, Plus } from 'lucide-react';
  ...
  return (
    <div className="min-h-screen bg-gray-50 text-[#0A0A0A] font-sans selection:bg-[#f9a602] selection:text-black">
      ...
      {/* =====================================================================================
          ...
          - Fixed Footer above Nav Bar (bottom-16 / bottom-17)
          - No extra whitespace in order card (h-fit)
      ...
      <div className="block md:hidden pb-32"> 
         <main className="px-4 py-6">
            ...
            <div className="flex justify-between items-center mb-6">
               ...
               <div className="text-[10px] font-bold text-green-600 bg-green-50 px-2 py-1 rounded flex items-center gap-1">
                  <ShieldCheck size={10}/> 100% SECURE
               </div>
            </div>
            ...
            <section className="bg-white p-4 rounded-xl border border-gray-200 shadow-sm mb-4">
               <div className="flex justify-between items-center mb-3">
                  ...
                  <span className="text-[10px] font-bold text-[#f9a602] cursor-pointer">CHANGE ADDRESS</span>
               </div>
               <div className="flex gap-3 items-start">
                  <MapPin size={16} className="mt-0.5 text-gray-400"/>
                  <div>
                     <span className="font-bold text-sm block">Home</span>
                     <p className="text-xs text-gray-500 leading-relaxed">Skyline Apts, Kakkanad</p>
                  </div>
               </div>
            </section>
            ...
            {/* C. Cart Items (Updated with h-fit to remove whitespace) */}
            <section className="bg-white p-4 rounded-xl border border-gray-200 shadow-sm h-fit">
               <h3 className="font-bold text-xs mb-4">Your Order ({totalQty})</h3>
                    ...
                    <div key={item.id} className="flex gap-3 border-b border-gray-50 pb-4 last:border-0 last:pb-0">
                       ...
                       <div className="flex-1">
                          ...
                          <div className="flex items-center gap-2">
                             <button onClick={() => decrementQty(item.id)} className="bg-gray-100 w-5 h-5 flex items-center justify-center rounded text-xs">-</button>
                             <span className="text-xs font-bold">{item.qty}</span>
                             <button onClick={() => incrementQty(item.id)} className="bg-gray-100 w-5 h-5 flex items-center justify-center rounded text-xs">+</button>
                          </div>
                       </div>
                    </div>
                  ))}
         ...
         {/* 🔴 MOBILE STICKY FOOTER (Positioned at bottom-16 to avoid nav bar) */}
         <div className="fixed bottom-16 left-0 w-full bg-white border-t border-gray-200 p-4 flex justify-between items-center z-40 shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.1)]">
            ...
            <button className="bg-[#f9a602] text-black px-8 py-3 rounded-lg text-sm font-bold shadow-lg hover:bg-[#0A0A0A] hover:text-white transition-colors uppercase tracking-wide">
               CONFIRM ORDER
            </button>
         </div>
      </div>
         ...
         <main className="px-8 py-10 max-w-3xl mx-auto">
            ...
            <div className="grid grid-cols-2 gap-6">
               <div className="space-y-6">
                  ...
                  <div className="bg-white p-5 rounded-2xl shadow-sm border border-gray-200">
                     <div className="flex justify-between items-center mb-4">
                        <h2 className="font-bold text-sm uppercase text-gray-400">Address</h2>
                        <button className="text-xs font-bold text-[#f9a602] hover:text-black transition-colors">CHANGE</button>
                     </div>
                     <div className="border border-[#f9a602] bg-[#f9a602]/5 p-4 rounded-xl flex items-center gap-3">
                        <MapPin size={20} className="text-[#0A0A0A]"/>
                        <div>
                           <p className="font-bold text-sm">Home</p>
                           <p className="text-xs text-gray-600">Skyline Apartments</p>
                        </div>
                     </div>
                  </div>
                  {/* Payment */}
               ...
               <div className="bg-white p-5 rounded-2xl shadow-sm border border-gray-200 h-fit">
                  ...
                  <button className="w-full bg-[#f9a602] text-black py-2 rounded-lg font-bold text-xs shadow-md hover:bg-black hover:text-[#f9a602] transition-colors uppercase tracking-wide">
                     Confirm Order
                  </button>
               </div>
            </div>
         ...
         <main className="max-w-6xl mx-auto px-8 py-12">
            ...
            <div className="grid grid-cols-12 gap-12">
               <div className="col-span-7 space-y-8">
                  ...
                  <section>
                     <div className="flex justify-between items-center mb-4">
                        <h2 className="text-sm font-black uppercase tracking-wider">1. Delivery Address</h2>
                        <button className="text-xs font-bold text-[#f9a602] hover:underline">CHANGE ADDRESS</button>
                     </div>
                     <div className="flex gap-4 items-start p-5 bg-white rounded-xl border border-gray-200 shadow-sm">
                        ...
                        <div>
                           <h3 className="font-bold text-sm">Home Address</h3>
                           <p className="text-sm text-gray-500 mt-1">Vnayak, Skyline Apartments, Kakkanad, Ernakulam - 682030</p>
                        </div>
                     </div>
                  </section>
                  {/* Payment */}
               ...
               <div className="col-span-5">
                  <div className="bg-white p-6 rounded-2xl border border-gray-200 shadow-lg sticky top-6">
                     ...
                     <div className="space-y-2 mb-6 text-sm">
                        ...
                        <div className="flex justify-between text-gray-500"><span>Delivery</span><span>₹{deliveryFee}</span></div>
                     ...
                     <button className="w-full bg-[#f9a602] text-black py-3.5 rounded-lg font-bold text-sm hover:bg-[#0A0A0A] hover:text-white transition-all flex justify-center items-center gap-2 shadow-md uppercase tracking-wide">
