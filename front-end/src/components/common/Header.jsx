      ...
      <div className="rounded-b-2xl md:hidden sticky top-0 z-[100] transition-all duration-300" style={{ background: "linear-gradient(180deg, #f9a602 0%, #fffbeb 60%, #ffffff 100%)" }}>
        ...
        <nav className="fixed bottom-0 left-0 right-0 z-[100] bg-white border-t border-gray-100 shadow-[0_-10px_30px_rgba(0,0,0,0.08)] px-2 py-1 flex items-center justify-around pb-4">
          {mobileNavLinks.slice(0, 2).map((link) => {
            ...
            return (
              <Link key={link.path} to={link.path} className="relative flex flex-col items-center justify-center w-14 h-12 rounded-xl transition-all duration-300 hover:bg-gray-100">
                <AnimatePresence>{isActive && <motion.div layoutId="mobileNavPill" className="absolute inset-0 bg-primary rounded-xl border border-black/5 shadow-sm" transition={{ type: "spring", stiffness: 350, damping: 30 }} />}</AnimatePresence>
                <div className="relative z-10 flex flex-col items-center">
                  <div className={`transition-transform duration-300 ${isActive ? "text-black scale-110" : "text-gray-600"}`}>{isActive ? displayActiveIcon : displayIcon}</div>
                  <span className={`text-[9px] font-black mt-0.5 transition-colors duration-300 ${isActive ? "text-black" : "text-gray-600"}`}>{link.name}</span>
                </div>
              </Link>
            );
          })}
          ...
          {mobileNavLinks.slice(2, 4).map((link) => {
            ...
            return (
              <Link key={link.path} to={link.path} className="relative flex flex-col items-center justify-center w-14 h-12">
                <AnimatePresence>{isActive && <motion.div layoutId="mobileNavPill" className="absolute inset-0 bg-primary rounded-xl border border-black/10 shadow-sm" transition={{ type: "spring", stiffness: 350, damping: 30 }} />}</AnimatePresence>
                <div className="relative z-10 flex flex-col items-center">
        ...
        <header className="bg-white rounded-b-2xl shadow-xl w-[95%] mx-auto flex flex-col items-center overflow-visible border border-gray-100">
          <div className="max-w-[1440px] h-20 md:h-24 lg:h-28 mx-auto px-4 md:px-6 lg:px-10 w-full flex items-center justify-between">
            <div className="flex items-center gap-2 lg:gap-4 shrink-0">
              ...
              {/* LOCATION: Shown ONLY on Large Desktops (xl) to prevent overflow */}
          ...
          {/* TABLET/LAPTOP (1024px) LOCATION BAR: Visible only between md and xl */}
          <div className="hidden md:flex xl:hidden w-full px-10 pb-4 justify-start">
            <div className="flex items-center gap-2 text-gray-500 cursor-pointer hover:text-black transition-colors">
              <MapPin size={14} className="text-primary fill-primary/20" />
              <span className="text-xs font-bold text-gray-700">Kakkanad, Ernakulam, Kerala</span>
